...
MIN_RECONSTRUCTION_PSNR = 28.0
MAX_ACCEPTABLE_WORST_ATTACK_PSNR = 22.0

CHECKPOINT_DIR = "checkpoints/share2_privacy"
OUTPUT_DIR = "outputs/share2_privacy"
...
def load_models():
    ...
    if not os.path.exists(
        ...
        raise FileNotFoundError(
            f"Missing encoder checkpoint: {encoder_path}"
        )
    ...
    if not os.path.exists(
        ...
        raise FileNotFoundError(
            f"Missing decoder checkpoint: {decoder_path}"
        )
...
def should_save_checkpoint(
    ...
    current_share2 = (
        metrics["share2_attack_psnr"]
    )

    best_share2 = (
        best_metrics["share2_attack_psnr"]
    )
    ...
    best_recon = (
        best_metrics["reconstruction_psnr"]
    )

    current_good_recon = (
        current_recon
        >= MIN_RECONSTRUCTION_PSNR
    )
    ...
    if current_good_recon and not best_good_recon:
        return True

    if not current_good_recon and best_good_recon:
        return False

    if current_worst < best_worst - 0.05:
        return True

    if abs(
        current_worst - best_worst
    ) <= 0.05:
        if current_share2 < best_share2 - 0.05:
            return True

        if abs(
            current_share2 - best_share2
        ) <= 0.05:
            return (
                current_recon
                > best_recon
...
def main():
    print("Device:", DEVICE)
    print()
    print("Share 2 targeted privacy GAN training")
    ...
    print(
        "Average-share weight:",
        AVERAGE_SHARE_WEIGHT
    )
    print(
        "Worst-share GAN weight:",
        WORST_GAN_WEIGHT
    )
    print(
    ...
    print(
        "Target Share 2 attack PSNR:",
        "< 20 dB preferred"
    )
    print(
        "Target worst-case attack PSNR:",
        "<= 22 dB"
    )
    print(
        "Minimum reconstruction target:",
        f"{MIN_RECONSTRUCTION_PSNR:.2f} dB"
    )
    print()
    ...
    for epoch in range(
        ...
        for batch_index, (images, _) in enumerate(
            ...
            if batch_index % 100 == 0:
                print(
                    ...
                    f"Share2Attack: {share2_attack_loss.item():.6f} "
        ...
        print(
            f"Train Share 2 Attack Loss: "
            f"{total_share2_attack / batches:.6f}"
        )
        ...
        print(
            f"Share 2 Attack PSNR: "
            f"{metrics['share2_attack_psnr']:.2f} dB"
        )
        ...
        for index, stats in enumerate(
            ...
            print(
                ...
                f"Std: {stats['std']:.5f} | "
                f"H-Corr: "
                f"{stats['horizontal_correlation']:.6f} | "
                f"V-Corr: "
                f"{stats['vertical_correlation']:.6f}"
            )
        ...
        if should_save_checkpoint(
            ...
            print(
                "New best Share 2 targeted checkpoint saved."
            )
            print(
                f"Best worst-case attack: "
                f"{metrics['worst_attack_psnr']:.2f} dB"
            )
            print(
                f"Best Share 2 attack: "
                f"{metrics['share2_attack_psnr']:.2f} dB"
            )
            print(
                f"Reconstruction: "
    ...
    print("=" * 70)
    print(
        "SHARE 2 TARGETED PRIVACY GAN TRAINING COMPLETE"
    )
    print("=" * 70)
    ...
    if best_metrics is not None:
        ...
        print(
            f"Best Share 2 attack PSNR: "
            f"{best_metrics['share2_attack_psnr']:.2f} dB"
        )
        print(
        ...
        print("Final share attack PSNRs:")
        ...
        privacy_good = (
            best_metrics["worst_attack_psnr"]
            <= MAX_ACCEPTABLE_WORST_ATTACK_PSNR
        )

        print(
            "Reconstruction target:",
            "PASSED" if reconstruction_good
            else "NOT PASSED"
        )

        print(
            "Worst-case privacy target:",
            "PASSED" if privacy_good
            else "NOT PASSED"
        )
